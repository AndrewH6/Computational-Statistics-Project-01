####################################################################################################
data(stanford2)
data_stan <- filter(stanford2,time >= 10)
data_stan$age2 <- data_stan$age^2

stan.fit <- lm(log10(time) ~ age + age2,data = data_stan)
# coefficients(stan.fit)
# names(data_stan)
stan_err <- stan.fit$residuals
...
for(i in 1:500){
  ...
  sage2 <- sage*sage
  err <- as.vector(base::sample(stan_err, 300,
                      replace = TRUE))
...
write.csv(datap_nlminb, "dataD-nlminb-bst.csv") 